...
- При запуске на сервере определите переменную окружения `ALLOWED_HOSTS` и положите туда ip-адрес сервера. Если сайт запускается локально, создавать переменную не нужно.
